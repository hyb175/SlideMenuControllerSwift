...
//  Modified by Nadav G on 08/09/16
//
...
public class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    private func isLeftPointContainedWithinBezelRect(_ point: CGPoint) -> Bool{
        if let bezelWidth = SlideMenuOptions.leftBezelWidth {
            let sliceResult: CGRect =  view.bounds.divided(atDistance: bezelWidth, from: CGRectEdge.minXEdge).slice
            //            view.bounds.divided(slice: &leftBezelRect, remainder: &tempRect, amount: bezelWidth, edge: CGRectEdge.minXEdge)
            
            return sliceResult.contains(point)
        } else {
            return true
        }
    }
